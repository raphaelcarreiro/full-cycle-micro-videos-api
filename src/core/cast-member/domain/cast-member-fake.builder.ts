...
export class CastMemberFakeBuilder<TBuild = any> {
  ...
  private _type: PropOrFactory<CastMemberType> = () => this.chance.integer({ min: 1, max: 2 }) as CastMemberType;
  ...
  static theCastMembers(quantity: number) {
    return new CastMemberFakeBuilder<CastMember[]>(quantity);
  }
  ...
  build(): TBuild {
    const castMembers = new Array(this.quantity).fill(undefined).map((_, index) => {
    ...
    return this.quantity === 1 ? (castMembers[0] as any) : castMembers;
  }
